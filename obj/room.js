...
room.prototype.leave = function(socket)
{
    ...
    if (indexOfUser > -1)
    {
        chat_room.sockets.in(this.roomName).emit('remove-user', {userId: this.safeUsers[indexOfUser].id});
...
room.prototype.join = function(socket)
{
    ...
    var safeUser = {id: socket.info.hashedId, ip: socket.info.hashedIp, username: socket.info.username, permissions: socket.info.permissions, room: socket.info.room, loggedin: socket.info.loggedin};
    ...
    socket.emit('userlist', {userlist: this.safeUsers}); //NOTE: session ID is sent with this, be sure that sessionIDs are worthless (Turns out they are not, you can cause disconnects and steal user data with them using xhr-polling
    socket.emit('room-event', {action: "playlistlock", data: this.playListLock});
    socket.broadcast.to(this.roomName).emit('add-user', {user: safeUser});
...
room.prototype.chatmessage = function(socket, message)
{
    ...
    chat_room.sockets.in(this.roomName).emit('chat', {user: {id: socket.info.hashedId, username: socket.info.username, permissions: socket.info.permissions, room: socket.info.room, loggedin: socket.info.loggedin, ip: socket.info.hashedIp}, message: message});
...
room.prototype.rename = function(socket, newname)
{
    ...
    if ((this.indexOfUser(newname) === -1) && newname.match(/^([A-Za-z0-9]|([-_](?![-_]))){1,16}$/) != null) //is name already taken?
    {
        ...
        chat_room.sockets.in(this.roomName).emit('rename', {username: newname, id: socket.info.hashedId});
    } 
};
